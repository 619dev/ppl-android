...
5. Tag the exact reviewed commit: `git tag -s v3.0.8 -m "PaperPhoneLite 3.0.8"`
   and then push that tag. If `v3.0.8` already exists, do not move it: release a
...
4. Verify that the public `v3.0.8` tag exists and resolves to the intended
   source. Replace `commit: v3.0.8` with the immutable full commit hash if the
