...
5. Tag the exact reviewed commit: `git tag -s v3.0.23 -m "PaperPhoneLite 3.0.23"`
   and then push that tag. If `v3.0.23` already exists, do not move it: release a
...
entry. The current candidate is `3.0.23` (`versionCode 30023`) at the immutable
`v3.0.23` tag declared by the metadata. Its signed GitHub build must pass
...
4. Verify that public tag `v3.0.23` resolves to the same immutable commit
